...
public final class DirectRuntimeRequestValidator implements RuntimeRequestValidator {
  ...
  DirectRuntimeRequestValidator(CConfiguration cConf, TransactionRunner txRunner,
    ...
    this.programRunsCache = CacheBuilder.newBuilder()
      ...
      .build(new CacheLoader<ProgramRunId, ImmutablePair<Boolean, RunRecordStatus>>() {
         ...
         */
        @Override
        public ImmutablePair<Boolean, RunRecordStatus> load(ProgramRunId programRunId) throws IOException,
  ...
  public RunRecordStatus checkProgramRunStatus(ProgramRunId programRunId, HttpRequest request) throws
    ...
    ImmutablePair<Boolean, RunRecordStatus> pair;
    try {
      pair = programRunsCache.get(programRunId);
    } catch (Exception e) {
  ...
  private ImmutablePair<Boolean, RunRecordStatus> getRunRecordStatusForProgramsInNonEndState(ProgramRunId programRunId)
    ...
    try {
      ...
      if (runRecord != null) {
        return runRecord.getStatus().isEndState() ? getInvalidRunRecordStatusImmutablePair() :
          getValidBooleanRunRecordStatusImmutablePair(runRecord);
      }
  ...
  private ImmutablePair<Boolean, RunRecordStatus> getValidBooleanRunRecordStatusImmutablePair(RunRecordDetail
                                                                                                runRecord) {
